...
$fotopemb = $this->db->get_where('tbl_pemb')->row();
$fotopembind = $this->db->get_where('tbl_pembind')->row();
$pesan = $this->db->get_where('chat')->row();
?>
<!-- Main content -->
                ...
                <table class="table datatable-basic" width="100%">
                  ...
                  <tbody>
                      ...
                      $no = 1;
                        ...
                        $cek_pemb = $this->db->get_where('tbl_penempatan', array('kdind'=>$baris->kdind, 'kdpemb'=>$user->row()->kdpemb))->row();
                        if (!empty($cek_pemb)){
                      ?>
                        <tr>
                          <td><?php echo $no.'.'; ?></td>
                          ...
                          <td>
                            <a href="javascript:;" data-friend="<?= $baris->kdpembind ?>" class="btn btn-success btn-xs"><i class="icon-comment"></i></a>
                          </td>
                        </tr>
                      <?php
                      $no++;
...
<div id="wgt-container-template" style="display: none">
    <div class="msg-wgt-container">
        ...
        <div class="msg-wgt-message-form">
            <textarea name="message" placeholder="Ketik pesan Anda. Tekan Shift + Enter untuk baris baru"></textarea>
        </div>
    </div>
</div>
    ...
    var initializeChat = function() {
        ...
        $('.msg-wgt-active').each(function(index, el) {
            ...
            chatInterval.push(setInterval(function() {
                ...
                var oldLength = 0;
                $.post('<?= site_url('users/getChats_pemb') ?>', {chatWith: $data.chatWith}, function(data, textStatus, xhr) {
                    ...
                    $.each(data.chats, function(index, el) {
                        ...
                        if ($that.find('#'+ id).length == 0) {
                            ...
                            tplBody.find('td.foto').html('<img src=\"'+val.sender_foto+'\">');
                            tplBody.find('td.name').text(val.sender_name); // set name
            ...
            $that.find('textarea').on('keydown', function(e) {
                ...
                if (e.keyCode === 13 && e.shiftKey === false) {
                    $.post('<?= site_url('users/sendMessage_pemb') ?>', {message: $textArea.val(), chatWith: $data.chatWith}, function(data, textStatus, xhr) {
                    });
