                ...
                <table class="table datatable-basic" width="100%">
                  ...
                  <tbody>
                      ...
                      foreach ($v_pemb->result() as $baris) {
                        $cek_pemb = $this->db->get_where('tbl_penempatan', array('kdpemb'=>$baris->kdpemb, 'kdind'=>$user->row()->kdind))->row();
                        if (!empty($cek_pemb)){
                      ?>
                        <tr>
                          <td><?php echo $no.'.'; ?></td>
    ...
    var initializeChat = function() {
        ...
        $('.msg-wgt-active').each(function(index, el) {
            ...
            chatInterval.push(setInterval(function() {
                ...
                var oldLength = 0;
                $.post('<?= site_url('users/getChats') ?>', {chatWith: $data.chatWith}, function(data, textStatus, xhr) {
            ...
            $that.find('textarea').on('keydown', function(e) {
                ...
                if (e.keyCode === 13 && e.shiftKey === false) {
                    $.post('<?= site_url('users/sendMessage') ?>', {message: $textArea.val(), chatWith: $data.chatWith}, function(data, textStatus, xhr) {
                    });
